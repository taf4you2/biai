...
def run_participant_image(args):
    ...
    run_command(
        [
            ...
            str(results_parent),
            "--output",
            str(results_parent / "participant_image_summary.csv"),
        ]
    )


def parse_args():
    parser = argparse.ArgumentParser(description="Run EEGNet held-out participant + held-out image evaluation.")
    parser.add_argument("--dataset-dir", default="event_epoch_multisession_image_on_0_0p8")
    parser.add_argument("--results-parent", default="eegnet_participant_image_results")
    parser.add_argument("--epochs", type=int, default=12)
    parser.add_argument("--batch-size", type=int, default=256)
    parser.add_argument("--test-size", type=float, default=0.2)
    parser.add_argument("--val-size", type=float, default=0.2)
    parser.add_argument("--val-split", choices=["auto", "random", "series", "participant"], default="participant")
    parser.add_argument("--label-control", choices=["none", "permute"], default="none")
    parser.add_argument("--normalization", choices=["global", "participant", "epoch"], default="participant")
    parser.add_argument("--balanced-sampler", choices=["none", "category", "participant", "category_participant"], default="category_participant")
...
if __name__ == "__main__":
    run_participant_image(parse_args())
